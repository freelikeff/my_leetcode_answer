...
class Solution:
    def ladderLength(self, beginWord: str, endWord: str, wordList: List[str]) -> int:
        ...
        length = len(wordList)
        connect = [[False] * length for _ in range(length)]
        ...
        visit = [0] * length
        deque_positive = [(start, 1)]
        deque_negetive = [(end, -1)]
        visit[start] = 1
        visit[end] = -1

        while deque_positive and deque_negetive:
            node, level = deque_positive.pop(0)
            for i in range(length):
                if not visit[i]:
                    if connect[node][i]:
                        visit[i] = level + 1
                        deque_positive.append((i, level + 1))
                elif visit[i] < 0:
                    if connect[node][i]:
                        return level - visit[i]

            node, level = deque_negetive.pop(0)
            for i in range(length):
                if not visit[i]:
                    if connect[node][i]:
                        visit[i] = level - 1
                        deque_negetive.append((i, level - 1))
                elif visit[i] > 0:
                    if connect[node][i]:
                        return visit[i] - level
        return 0
...
class Solution2:
    def ladderLength(self, beginWord: str, endWord: str, wordList: List[str]) -> int:
        ...
        start_set = {beginWord}
        end_set = {endWord}
        letters, length = set('qwertyuiopasdfghjklzxcvbnm'), len(beginWord)
        step = 1
        while start_set:
            step += 1
            temp_set = set()
            ...
            if len(temp_set) < len(end_set):
                start_set = temp_set
            else:
                start_set, end_set = end_set, temp_set
        return 0


if __name__ == '__main__':
    s = Solution()
    # inpt = "hit", "cog", ["hot", "dot", "dog", "lot", "log", "cog"]
    inpt = "sand", "acne", ["slit", "bunk", "wars", "ping", "viva", "wynn", "wows", "irks", "gang", "pool", "mock",
