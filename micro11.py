...
class Solution:
    # 镜像对称转换
    def MirrorRecursively(self, pRootOfTree: TreeNode) -> TreeNode or None:
        if not pRootOfTree:
            return None

        pRootOfTree.left, pRootOfTree.right = self.MirrorRecursively(pRootOfTree.right), self.MirrorRecursively(
            pRootOfTree.left)
        return pRootOfTree
